...
public:
    vector<int> maxActiveSectionsAfterTrade(string s, vector<vector<int>>& queries) {
        ...
        // example : s = "11000011" , answer = simply count 1s "activeCount"
        if (m < 2) {
        ...
        vector<int> result;
        for (auto &q : queries) { // O(q * log(n))
            int l = q[0];
            int r = q[1];

            // first block in this range
            int low = lower_bound(begin(blockEnd), end(blockEnd), l) - begin(blockEnd);  // O(log)

            int high = upper_bound(begin(blockStart), end(blockStart), r) - begin(blockStart) - 1; // O(log)
